...
// 
// buttons.c
// Author:  John He, Matthew Ting and Suhao Zhang
// Last modified:   28.05.2018
//
// Support for a set of FOUR specific buttons on the Tiva/Orbit.
// The buttons are:  UP, DOWN, RIGHT SWITCH, and RESeT (on the Orbit daughterboard) plus
...
initButtons (void)
{
    ...
    // SWITCH (active HIGH)
        SysCtlPeripheralEnable (SWITCH_RIGHT_PERIPH);
        GPIOPinTypeGPIOInput (SWITCH_RIGHT_PORT_BASE, SWITCH_RIGHT_PIN);
        GPIOPadConfigSet (SWITCH_RIGHT_PORT_BASE, SWITCH_RIGHT_PIN, GPIO_STRENGTH_2MA,
           GPIO_PIN_TYPE_STD_WPD);
        but_normal[SWITCH_RIGHT] = SWITCH_RIGHT_NORMAL;
    ...
    // Read initial state of RESET Button and the S1 SWITCH
    but_state[RESET] = (GPIOPinRead (RESET_BUT_PORT_BASE, RESET_BUT_PIN) == RESET_BUT_PIN);
    but_state[SWITCH_RIGHT] = (GPIOPinRead (SWITCH_RIGHT_PORT_BASE, SWITCH_RIGHT_PIN) == SWITCH_RIGHT_PIN);
}
...
updateButtons (void)
{
    ...
    but_value[SWITCH_RIGHT] = (GPIOPinRead (SWITCH_RIGHT_PORT_BASE, SWITCH_RIGHT_PIN) == SWITCH_RIGHT_PIN);
